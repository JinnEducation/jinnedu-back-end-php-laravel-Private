<x-front-layout>
    ...
    @php
        // Helpers safe
        $title = $courseLang->title ?? $course->title ?? 'Course';
        $subtitle = $courseLang->subtitle ?? $course->subtitle ?? '';
        $about = $courseLang->description ?? $course->description ?? '';
        $requirements = $course->requirements ?? []; // array أو json
        $learnItems = $course->learning_outcomes ?? []; // array أو json
        $hoursText = $course->total_hours ? ($course->total_hours . ' hours') : null;
        $levelText = $course->level ?? null;

        // أسعار
        $oldPrice = $course->price_old ?? null;
        $price = $course->price ?? null;

        // صورة/فيديو
        $cover = $course->cover_image
            ? asset($course->cover_image)
            : asset('front/assets/imgs/Rectangle 1904355.png');

        // شهادة
        $certificateImage = $course->certificate_image
            ? asset($course->certificate_image)
            : asset('front/assets/imgs/cer1.jpg');

        // video
        $previewVideoUrl = $course->preview_video_url ?? null;

        // تأكد من arrays إذا كانت json string
        if (is_string($requirements)) {
            $decoded = json_decode($requirements, true);
            $requirements = is_array($decoded) ? $decoded : [];
        }
        if (is_string($learnItems)) {
            $decoded = json_decode($learnItems, true);
            $learnItems = is_array($decoded) ? $decoded : [];
        }
    @endphp
        ...
        <div class="container z-10 px-4 mx-auto w-full">
            ...
            <nav aria-label="Breadcrumb" class="mb-6">
                <div class="text-sm font-light text-black leading-relaxed">
                    <span>
                        <a href="{{ url('/') }}" class="text-primary-600 hover:text-primary-700">Home</a>
                    </span>
            ...
            <div>
                <h1 class="text-3xl font-bold text-black mb-3">{{ $title }}</h1>
                @if($subtitle)
                    <p class="text-2xl text-black leading-relaxed">{{ $subtitle }}</p>
                @endif
            </div>
        </div>
    </section>
                ...
                <div class="w-full lg:w-2/3 space-y-10">
                    ...
                    <div id="about-course">
                        ...
                        <p class="text-[15px] text-black leading-7">
                            {{ $about ?: 'No description available yet.' }}
                        </p>
                    </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                @foreach($learnItems as $item)
                                    <div class="flex items-start gap-2">
                                        ...
                                        <p class="text-[15px] text-black leading-6">{{ $item }}</p>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <p class="text-[15px] text-black leading-7">No outputs added yet.</p>
                        @endif
                    </div>
                    ...
                    <div id="course-content" class="mt-6 border border-gray-300 p-4">
                        <div class="flex items-center justify-between gap-2 mb-4">
                            ...
                            <span class="text-[14px] text-gray-500">
                                {{ $hoursText ?? '—' }}
                                @if($levelText) • {{ $levelText }} @endif
                            </span>
                        </div>

                        <div class="space-y-2">
                            @forelse(($course->units ?? []) as $unitIndex => $unit)
                                @php
                                    $unitTitle = $unit->title ?? ('Unit ' . ($unitIndex + 1));
                                    $lessons = $unit->lessons ?? collect();
                                    $isOpen = $unitIndex === 0;
                                @endphp
                                ...
                                <div class="accordion-item border border-gray-200 rounded-lg overflow-hidden">
                                    <button
                                        ...
                                        <svg class="icon w-4 h-4 text-gray-500 transition-transform {{ $isOpen ? 'rotate-180' : '' }}"
                                             fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                  d="{{ $isOpen ? 'M20 12H4' : 'M12 4v16m8-8H4' }}"></path>
                                        </svg>
                                    </button>

                                    <div class="accordion-body {{ $isOpen ? '' : 'hidden' }} px-2 py-3 text-lg text-black leading-6">
                                        @if($lessons->count())
                                            <div class="space-y-2">
                                        ...
                                        @else
                                            <p class="text-[15px] text-gray-700">No lessons in this unit yet.</p>
                                        @endif
                                    </div>
                                </div>
                            @empty
                                <p class="text-[15px] text-gray-700">No units added yet.</p>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div id="requirements" class="mt-10">
                        ...
                        @php
                            $req = $requirements;
                            if (empty($req) && is_string($course->requirements_text ?? null)) {
                                $req = array_filter(array_map('trim', preg_split("/\r\n|\n|\r/", $course->requirements_text)));
                            }
                        @endphp
                        ...
                        @if(!empty($req))
                            <div class="space-y-2">
                                @foreach($req as $r)
                                    <div class="flex items-center gap-2">
                                        <span class="w-1 h-1 rounded-full bg-black mt-1.5 flex-shrink-0"></span>
                                        <p class="text-[15px] text-black leading-6">{{ $r }}</p>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <p class="text-[15px] text-gray-700">No requirements added yet.</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="w-full lg:w-1/3 space-y-4 mt-8 lg:mt-0">
                    <div class="rounded-md bg-white shadow-lg">
                        ...
                        <div class="relative rounded-t-lg w-full h-[216px] bg-gray-200 overflow-hidden">
                            <img src="{{ $cover }}" alt="Course video" class="w-full h-full object-cover">

                            <span class="absolute top-1 right-1 rtl:left-1 rtl:right-auto text-sm bg-primary text-white p-1.5 rounded-lg z-8">
                                {{ $course->has_certificate ? 'Certified Achievement Certificate' : 'Course Preview' }}
                            </span>

                            @if($previewVideoUrl)
                                <div class="absolute inset-0 flex items-center justify-center z-8">
                                    <a href="{{ $previewVideoUrl }}" target="_blank" class="bg-primary px-3 py-0.5 rounded-lg">
                                        <svg class="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M8 5v14l11-7z"></path>
                                        </svg>
                                    </a>
                                </div>
                                <a href="{{ $previewVideoUrl }}" target="_blank"
                                   class="absolute left-1/2 -translate-1/2 z-10 bottom-0 text-white font-bold text-sm">
                                    View the course
                                </a>
                            @else
                                <button class="absolute left-1/2 -translate-1/2 z-10 bottom-0 text-white font-bold text-sm" type="button">
                                    View the course
                                </button>
                            @endif
                        ...
                        <div class="p-5">
                            <p class="text-[13px] text-black font-bold mb-1">
                                {{ $course->is_in_plan ? 'This paid course is included in the plans' : 'Enroll in this course' }}
                            </p>

                            <div class="flex items-center gap-2">
                                @if($oldPrice)
                                    <span class="text-[17px] line-through text-gray-400">${{ $oldPrice }}</span>
                                @endif
                                <span class="text-[19px] font-bold text-primary">
                                    {{ $price !== null ? ('$' . $price) : 'Free' }}
                                </span>
                            </div>

                            @if($course->discount_label ?? false)
                                <div class="mb-3 mt-2">
                            ...
                            <div class="space-y-1 text-[13px] text-black font-light mb-4">
                                ...
                                @empty
                                    <div class="flex items-start gap-2">
                                        <span class="w-1 h-1 rounded-full bg-black mt-1.5 flex-shrink-0"></span>
                                        <span>{{ $course->total_hours ? ($course->total_hours . ' hours of on-demand video') : 'On-demand course content' }}</span>
                                    </div>
                                    <div class="flex items-start gap-2">
                                        <span class="w-1 h-1 rounded-full bg-black mt-1.5 flex-shrink-0"></span>
                        ...
                        <div class="flex items-center gap-2 mb-2">
                            <span>
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    ...
                                    <path d="M5 14.5C6.38071 14.5 7.5 13.3807 7.5 12C7.5 10.6193 6.38071 9.5 5 9.5C3.61929 9.5 2.5 10.6193 2.5 12C2.5 13.3807 3.61929 14.5 5 14.5Z"
                                          stroke="#1B449C" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" />
                                    <path d="M19 7.5C20.3807 7.5 21.5 6.38071 21.5 5C21.5 3.61929 20.3807 2.5 19 2.5C17.6193 2.5 16.5 3.61929 16.5 5C16.5 6.38071 17.6193 7.5 19 7.5Z"
                                          stroke="#1B449C" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" />
                                    <path d="M19 21.5C20.3807 21.5 21.5 20.3807 21.5 19C21.5 17.6193 20.3807 16.5 19 16.5C17.6193 16.5 16.5 17.6193 16.5 19C16.5 20.3807 17.6193 21.5 19 21.5Z"
                                          stroke="#1B449C" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" />
                                    <path d="M7.29999 11L16.7 6M7.29999 13L16.7 18"
                                          stroke="#1B449C" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                            </span>
                            <p class="text-[13px] font-light text-black">Share on your social media or LinkedIn</p>
                        </div>
                        <img src="{{ $certificateImage }}" alt="Certificate" class="w-full object-contain rounded-b-lg">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="container mx-auto px-4">
            ...
            <div id="course-reviews" class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        @for($s=5; $s>=1; $s--)
                            <div class="flex items-center gap-2 text-[14px] text-black">
                                <span class="flex items-center gap-2">
                                    ...
                                    <span class="font-bold text-[15px]">{{ $s }} stars</span>
                                </span>
                                <div class="flex-1 h-1.5 rounded bg-gray-200">
                                    <div class="bg-primary h-1.5 rounded" style="width: {{ $starPercents[$s] ?? 0 }}%"></div>
                                </div>
                                <span class="w-11 text-[#00000099]">{{ $starPercents[$s] ?? 0 }}%</span>
                            </div>
                        @endfor
                    </div>
                </div>
                ...
                <div class="space-y-4 md:col-span-2">
                    ...
                    @empty
                        <p class="text-[15px] text-gray-700">No reviews yet.</p>
                    @endforelse
                </div>
            </div>
        </div>
        ...
        <div class="container mx-auto">
            ...
            <div class="grid grid-cols-1 gap-6 mb-12 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" id="coursesGrid">
                @forelse($relatedCourses as $rc)
                    @php
                        $rcLang = $rc->langs?->firstWhere('lang', app()->getLocale()) ?? $rc->langs?->first();
                        $rcTitle = $rcLang->title ?? $rc->title ?? 'Course';
                        $rcDesc  = $rcLang->subtitle ?? $rc->subtitle ?? '';
                        $rcImg = $rc->cover_image ? asset($rc->cover_image) : 'https://images.unsplash.com/photo-1517180102446-f3ece451e9d8?w=400&h=250&fit=crop';
                        $rcPrice = $rc->price;
                        $rcOld = $rc->price_old;
                    @endphp
                    ...
                    <div class="block overflow-hidden p-3 bg-white rounded-md shadow-sm transition-all duration-300 group course-card hover:shadow-lg hover:scale-105">
                        <div class="overflow-hidden relative h-48 rounded-sm">
                            <img src="{{ $rcImg }}" alt="{{ $rcTitle }}" class="object-cover w-full h-full">
                        </div>

                        <div class="pt-4">
                            <h3 class="mb-2 text-lg font-bold text-black text-[18px]">{{ $rcTitle }}</h3>
                            <p class="mb-4 text-[13px] text-black">{{ $rcDesc }}</p>
                                ...
                                <div class="flex justify-between items-center h-[45px] transition-all duration-300 group-hover:opacity-0 group-hover:hidden">
                                    <div class="flex gap-2 items-center">
                                        ...
                                        <span class="text-sm text-black">
                                            {{ $rc->total_hours ? ($rc->total_hours . ' total hours') : '—' }}
                                        </span>
                                    </div>

                                    @if($rcPrice === null || $rcPrice == 0)
                                        <span class="text-lg font-bold text-[#1B449C]">Free</span>
                                    @else
                                        <div class="flex flex-col items-end">
                                            @if($rcOld)
                                                <span class="text-sm line-through text-[#87CEEB]">${{ $rcOld }}</span>
                                            @endif
                                            <span class="text-[15px] font-bold text-[#1B449C]">${{ $rcPrice }}</span>
                                        </div>
                                    @endif
                                </div>

                                <div class="hidden opacity-0 transition-all duration-300 group-hover:flex group-hover:opacity-100">
                                    <a href="{{ route('site.singlecourse', $rc->id) }}"
                ...
                @empty
                    <p class="text-[15px] text-gray-700">No related courses found.</p>
                @endforelse
            </div>
