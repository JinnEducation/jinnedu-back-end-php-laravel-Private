...
use App\Http\Requests\Tutor\TutorSignUpRequest;
...
class WalletController extends Controller
{
    ...
    public function refund($orderid){
    	...
    	if(isset($json->status) && $json->status=='APPROVED') {
    	    ...
    	    }else $wallet->balance += $refOrder->price;
    	    $wallet->save();
    ...
    public function transfer($conference_id){
        ...
        if(!$conferences) return response([
                'success' => false,
    ...
    $user->id
        	    ...
        	    //=====================================================================
                if($wallet->group_class_count>0) $wallet->group_class_count -=1; 
        	    else $wallet->balance -= $order->price;
                $wallet->save();
        	    //=====================================================================
    ...
    $user->id
            ...
            }else if($order->ref_type==7){
                $package = Post::where('id',$order->ref_id)->where('content_type',11)->first();
                if($package)  $package->info = $package->package()->first();
