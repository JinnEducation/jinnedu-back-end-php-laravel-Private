    ...
    public function getAssignedGroupClass(Request $request, $slug=null){
        ...
        if($slug!=null){
            ...
            if($items){
                $items->level;
                $items->category;
                if($items->category) $items->category->langs = $items->category->langs()->get();
                $items->langs = $items->langs()->get();
                $items->dates = $items->dates()->get();
                $items->reviews = $items->reviews()->get();
                if($items->reviews) foreach($items->reviews as $review) {
                    $review->user;
                ...
                if($items->tutor) $items->tutor->email=null;
                ...
                foreach($suggestions as $suggestion){
                    $suggestion->level;
                    $suggestion->category;
                    if($suggestion->category) $suggestion->category->langs = $suggestion->category->langs()->get();
                    ...
                    $suggestion->tutor;
                    if($suggestion->tutor) $suggestion->tutor->email=null;
            ...
            if(!empty($request->tutor_id)) $items->where('tutor_id',$request->tutor_id);

            if(!empty($request->category_id)) $items->where('category_id',$request->category_id);
            if(!empty($request->category)) $items->where('category_id',$request->category);
    ...
    public function setGroupClassLangs($item,$langs){
        ...
        if(!empty($langs) && count($langs)>0){
            foreach($langs as $langArray) {
                ...
                else GroupClassLang::where('classid',$item->id)->where('language_id',$lang->language_id)->update( $langData );
            }
        }
    }
    ...
    public function setGroupClassOutlines($item,$outlines){
        ...
        if(!empty($outlines) && count($outlines)>0){
            foreach($outlines as $outline) {
                ...
                if(!$checkOutline) $gco = GroupClassOutline::create($gcoData);
                else {
                ...
                if($gco) $this->setGroupClassDates($gco,$outline['dates']);
            }
        }
    }
